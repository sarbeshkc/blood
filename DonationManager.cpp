...
bool DonationManager::recordDonation(const QString &userEmail,
                                     double bloodAmount) {
  QSqlQuery query(m_db);
  query.prepare(
      "INSERT INTO blood_donations (user_id, donation_date, blood_amount) "
      "SELECT id, :donation_date, :blood_amount FROM users WHERE email = "
      ":email");
  ...
  if (query.exec()) {
    ...
    qCCritical(donationManager)
        << "Error inserting donation record:" << query.lastError().text();
    return false;
  }
}
...
QVariantList DonationManager::getUserDonationHistory(const QString &userEmail) {
  ...
  if (query.exec()) {
    ...
    qCCritical(donationManager)
        << "Error fetching donation history:" << query.lastError().text();
  }
...
QVariantList DonationManager::getRecentDonations(int limit) {
  ...
  QSqlQuery query(m_db);
  query.prepare(
      "SELECT u.name, u.blood_group, bd.donation_date, bd.blood_amount "
  ...
  if (query.exec()) {
    ...
    qCCritical(donationManager)
        << "Error fetching recent donations:" << query.lastError().text();
  }
...
QVariantMap DonationManager::getBloodInventory() {
  ...
  if (query.exec()) {
    ...
    qCCritical(donationManager)
        << "Error fetching blood inventory:" << query.lastError().text();
  }
...
QVariantList
DonationManager::getDetailedDonationHistory(const QString &userEmail) {
  QVariantList detailedHistory;
  QSqlQuery query(m_db);
  query.prepare(
      "SELECT bd.donation_date, bd.blood_amount, h.name as hospital_name "
      "FROM blood_donations bd "
      "JOIN users u ON bd.user_id = u.id "
      "JOIN appointments a ON a.user_id = u.id AND a.appointment_date = "
      "bd.donation_date "
  ...
  if (query.exec()) {
    ...
    qCCritical(donationManager) << "Error fetching detailed donation history:"
...
QString DonationManager::getLastDonationDate(const QString &userEmail) {
  QSqlQuery query(m_db);
  query.prepare("SELECT MAX(donation_date) as last_donation "
                "FROM blood_donations bd "
                "JOIN users u ON bd.user_id = u.id "
  ...
  if (query.exec() && query.next()) {
    return query.value("last_donation").toDateTime().toString("yyyy-MM-dd");
  } else {
    qCCritical(donationManager)
        << "Error fetching last donation date:" << query.lastError().text();
    return QString();
  }
}
